...
const PopularList = () => {
  ...
  const getProvinceAndAttractionId = (attractionId) => {
    ...
    const popularDataSmall = popularData.slice(0, 3);
    const popularDataBig = popularData.slice(0, 4);
  return (
    <>
      {
        (isSmallScreen || isMediumScreen) ?
          <div className="row d-md-none d-xl-inline-flex d-xxxl-none mx-auto">
            ...
            <p className={`lead ms-5 text-center ${popularData.length == 0 ? "" : "d-none"}​`}>{language === 'English' ? 'Popular places will be displayed here.' : 'កន្លែងពេញនិយមនឹងត្រូវបានបង្ហាញនៅទីនេះ។'}</p>
            {Array.from({ length: popularDataSmall.length }, (_, index) => (
              <Content {...popularDataSmall[index]} key={popularDataSmall[index].id} provinceData={provinceData} />
              ))}
          </div>
        :
          <div className="row d-none d-md-inline-flex d-xl-none d-xxxl-inline-flex d-xxxl-inline-flex mx-auto">
            <h1  className="text-center my-3 fw-bold">{language === 'English' ? 'Popular' : "កន្លែងពេញនិយម"}</h1>
            <p className={`lead ms-5 text-center ${popularData.length == 0 ? "" : "d-none"}`}>{language === 'English' ? 'Popular places will be displayed here.' : 'កន្លែងពេញនិយមនឹងត្រូវបានបង្ហាញនៅទីនេះ។'}</p>
            {Array.from({ length: popularDataBig.length }, (_, index) => (
              <Content {...popularDataBig[index]} key={popularDataBig[index].id} provinceData={provinceData} />
            ))}
          </div>
      }
    </>
